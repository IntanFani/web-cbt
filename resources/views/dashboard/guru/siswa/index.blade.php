...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <p class="text-muted">Total siswa terdaftar: <span class="badge bg-soft-primary text-primary">{{ $siswa->total() }} Siswa</span></p>
        </div>
            ...
            <form action="{{ route('siswa.index') }}" method="GET" class="row g-3 align-items-end">
                <div class="col-md-4">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" name="search" class="form-control bg-light border-0" placeholder="Nama atau email..." value="{{ request('search') }}">
                    </div>
                </div>
                ...
                <div class="col-md-2">
                    <button type="submit" class="btn btn-dark w-100 fw-bold rounded-3 py-2">
                        Filter Data
                    </button>
                </div>
            </form>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        <img src="https://ui-avatars.com/api/?name={{ urlencode($s->name) }}&background=random&color=fff" class="rounded-3 me-3" width="40">
                                        <div>
                                ...
                                <td>
                                    <div class="mb-1">
                                        <span class="badge bg-soft-info text-info px-3 py-2 rounded-pill" style="background: #e0f2fe;">
                                ...
                                <td>
                                    <span class="text-muted small"><i class="far fa-calendar-alt me-1"></i> {{ $s->created_at->format('d M Y') }}</span>
                                </td>
                                <td class="text-end pe-4">
                                    <div class="d-flex justify-content-end gap-2">
                                        <a href="{{ route('siswa.edit', $s->id) }}" class="btn btn-sm btn-outline-warning rounded-3 px-2" title="Edit Siswa">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        
                                        <button type="button" class="btn btn-sm btn-outline-danger rounded-3 px-2" onclick="confirmDelete({{ $s->id }}, '{{ $s->name }}')" title="Hapus Siswa">
                                            <i class="fas fa-trash-alt"></i>
                                        </button>
                                    </div>

                                    <form action="{{ route('siswa.delete', $s->id) }}" method="POST" class="d-none" id="delete-form-{{ $s->id }}">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </td>
